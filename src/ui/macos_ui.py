...
from ..core.network_manager import NetworkManager, TCP_CLIENT, TCP_SERVER, UDP
...
class MacOSSerialUI(QWidget):
    ...
    def __init__(self, config: AppConfig, serial_mgr: SerialManager):
        ...
        # 当前模式：'serial'、'bluetooth' 或 'network'
    ...
    def create_left_sidebar(self):
        ...
        self.mode_combo.addItems(["串口", "蓝牙", "网络"])
        self.mode_combo.currentTextChanged.connect(self.on_mode_changed)
        layout.addWidget(self.mode_combo)
        
        # 设备选择
        self.device_label = QLabel("设备")
        self.device_label.setObjectName("compactLabel")
        layout.addWidget(self.device_label)
        
        # 设备选择容器（包含下拉框和刷新按钮）
        self.device_container = QWidget()
        device_container = QHBoxLayout(self.device_container)
        device_container.setContentsMargins(0, 0, 0, 0)
        ...
        self.network_config_widget = QWidget()
        network_layout = QVBoxLayout(self.network_config_widget)
        network_layout.setContentsMargins(0, 0, 0, 0)
        network_layout.setSpacing(7)

        network_type_label = QLabel("协议")
        network_type_label.setObjectName("compactLabel")
        network_layout.addWidget(network_type_label)
        self.network_type_combo = QComboBox()
        self.network_type_combo.addItems(["TCP 客户端", "TCP 服务端", "UDP"])
        self.network_type_combo.setCurrentText(
            self.config.get("network.mode", "TCP 客户端")
        )
        network_layout.addWidget(self.network_type_combo)

        self.network_host_label = QLabel("目标主机")
        self.network_host_label.setObjectName("compactLabel")
        network_layout.addWidget(self.network_host_label)
        self.network_host_input = QLineEdit(
            self.config.get("network.host", "127.0.0.1")
        )
        self.network_host_input.setPlaceholderText("127.0.0.1")
        network_layout.addWidget(self.network_host_input)

        self.network_port_label = QLabel("目标端口")
        self.network_port_label.setObjectName("compactLabel")
        network_layout.addWidget(self.network_port_label)
        self.network_port_input = QLineEdit(str(self.config.get("network.port", 8080)))
        self.network_port_input.setValidator(QIntValidator(1, 65535, self))
        self.network_port_input.setPlaceholderText("8080")
        network_layout.addWidget(self.network_port_input)

        self.network_local_host_label = QLabel("本机绑定 IP")
        self.network_local_host_label.setObjectName("compactLabel")
        network_layout.addWidget(self.network_local_host_label)
        self.network_local_host_input = QLineEdit(
            self.config.get("network.local_host", "0.0.0.0")
        )
        self.network_local_host_input.setPlaceholderText("0.0.0.0（自动选择）")
        self.network_local_host_input.setToolTip("选择发送或监听所使用的本机网卡 IP")
        network_layout.addWidget(self.network_local_host_input)

        self.network_local_port_label = QLabel("本地端口")
        self.network_local_port_label.setObjectName("compactLabel")
        network_layout.addWidget(self.network_local_port_label)
        self.network_local_port_input = QLineEdit(
            str(self.config.get("network.local_port", 0))
        )
        self.network_local_port_input.setValidator(QIntValidator(0, 65535, self))
        self.network_local_port_input.setPlaceholderText("0（自动分配）")
        network_layout.addWidget(self.network_local_port_input)

        self.network_keepalive_check = QCheckBox("TCP 保活")
        self.network_keepalive_check.setChecked(
            self.config.get("network.keepalive", False)
        )
        self.network_keepalive_check.setToolTip("检测长时间无响应的 TCP 连接")
        network_layout.addWidget(self.network_keepalive_check)

        self.network_broadcast_check = QCheckBox("允许 UDP 广播")
        self.network_broadcast_check.setChecked(
            self.config.get("network.broadcast", False)
        )
        self.network_broadcast_check.setToolTip("允许向 255.255.255.255 等广播地址发送")
        network_layout.addWidget(self.network_broadcast_check)

        self.network_auto_reply_check = QCheckBox("收到消息自动回复")
        self.network_auto_reply_check.setChecked(
            self.config.get("network.auto_reply", False)
        )
        network_layout.addWidget(self.network_auto_reply_check)
        self.network_auto_reply_input = QLineEdit(
            self.config.get("network.auto_reply_text", "")
        )
        self.network_auto_reply_input.setPlaceholderText("自动回复内容")
        network_layout.addWidget(self.network_auto_reply_input)
        self.network_auto_reply_format = QComboBox()
        self.network_auto_reply_format.addItems(["ASCII", "UTF-8", "HEX"])
        self.network_auto_reply_format.setCurrentText(
            self.config.get("network.auto_reply_format", "ASCII")
        )
        network_layout.addWidget(self.network_auto_reply_format)

        self.network_client_label = QLabel("发送目标")
        self.network_client_label.setObjectName("compactLabel")
        network_layout.addWidget(self.network_client_label)
        self.network_client_combo = QComboBox()
        self.network_client_combo.addItem("所有已连接客户端", None)
        self.network_client_combo.setToolTip("TCP 服务端可选择单个客户端，默认广播")
        network_layout.addWidget(self.network_client_combo)
        layout.addWidget(self.network_config_widget)
        
        # 默认隐藏蓝牙配置
        self.bluetooth_config_widget.hide()
        self.network_config_widget.hide()
    ...
    def create_right_sidebar(self):
        ...
        self.unit_converter = UnitConverterWidget()
        tabs.addTab(self.unit_converter, "单位")
    ...
    def on_mode_changed(self, mode_text: str):
        ...
        if mode_text == "串口":
            ...
            self.serial_config_widget.hide()
            self.bluetooth_config_widget.hide()
            self.network_config_widget.show()
            self.device_label.hide()
            self.device_container.hide()
            self.refresh_btn.hide()
            self.line_control_card.hide()
            self._update_network_field_labels()
    ...
    def active_manager(self):
        """Return the active transport without spreading mode checks around UI code."""
        if self.current_mode == 'serial':
    ...
    def refresh_devices(self):
        ...
        if self.current_mode == 'serial':
            self.refresh_ports()
        elif self.current_mode == 'bluetooth':
    ...
    def connect_signals(self):
        ...
        self.network_manager.device_connected.connect(self.on_device_connected)
        self.network_manager.device_disconnected.connect(self.on_device_disconnected)
        self.network_manager.error_occurred.connect(self.on_error)
        self.network_manager.connecting_status.connect(self.on_connecting_status)
        self.network_manager.clients_changed.connect(self._update_network_clients)
        ...
        self.network_type_combo.currentTextChanged.connect(self._on_network_config_changed)
        self.network_host_input.editingFinished.connect(self._on_network_config_changed)
        self.network_port_input.editingFinished.connect(self._on_network_config_changed)
        self.network_local_host_input.editingFinished.connect(self._on_network_config_changed)
        self.network_local_port_input.editingFinished.connect(self._on_network_config_changed)
        self.network_keepalive_check.toggled.connect(self._on_network_config_changed)
        self.network_broadcast_check.toggled.connect(self._on_network_config_changed)
        self.network_auto_reply_check.toggled.connect(self._on_network_config_changed)
        self.network_auto_reply_input.editingFinished.connect(self._on_network_config_changed)
        self.network_auto_reply_format.currentTextChanged.connect(self._on_network_config_changed)
        self.network_client_combo.currentIndexChanged.connect(self._set_network_send_target)
    ...
    def _network_mode_key(self) -> str:
        return {
            "TCP 客户端": TCP_CLIENT,
            "TCP 服务端": TCP_SERVER,
            "UDP": UDP,
        }[self.network_type_combo.currentText()]

    def _update_network_field_labels(self):
        is_server = self._network_mode_key() == TCP_SERVER
        is_udp = self._network_mode_key() == UDP
        needs_local_binding = is_udp or self._network_mode_key() == TCP_CLIENT
        self.network_host_label.setText("监听地址" if is_server else "目标主机")
        self.network_port_label.setText("监听端口" if is_server else "目标端口")
        self.network_local_host_label.setVisible(needs_local_binding)
        self.network_local_host_input.setVisible(needs_local_binding)
        self.network_local_port_label.setVisible(needs_local_binding)
        self.network_local_port_input.setVisible(needs_local_binding)
        self.network_keepalive_check.setVisible(not is_udp)
        self.network_broadcast_check.setVisible(is_udp)
        can_auto_reply = is_server or is_udp
        self.network_auto_reply_check.setVisible(can_auto_reply)
        self.network_auto_reply_input.setVisible(can_auto_reply)
        self.network_auto_reply_format.setVisible(can_auto_reply)
        self.network_client_label.setVisible(is_server)
        self.network_client_combo.setVisible(is_server)
        self.network_host_input.setPlaceholderText("0.0.0.0" if is_server else "127.0.0.1")

    def _on_network_config_changed(self):
        self._update_network_field_labels()
        try:
            values = {
                "mode": self.network_type_combo.currentText(),
                "host": self.network_host_input.text().strip(),
                "port": int(self.network_port_input.text()),
                "local_host": self.network_local_host_input.text().strip() or "0.0.0.0",
                "local_port": int(self.network_local_port_input.text() or 0),
                "keepalive": self.network_keepalive_check.isChecked(),
                "broadcast": self.network_broadcast_check.isChecked(),
                "auto_reply": self.network_auto_reply_check.isChecked(),
                "auto_reply_text": self.network_auto_reply_input.text(),
                "auto_reply_format": self.network_auto_reply_format.currentText(),
            }
        except ValueError:
            return
        for key, value in values.items():
            self.config.set(f"network.{key}", value)
        self.config.save_config()

    def _update_network_clients(self, endpoints: list):
        """Keep the server target picker in sync with short-lived TCP clients."""
        selected = self.network_client_combo.currentData()
        blocker = QSignalBlocker(self.network_client_combo)
        self.network_client_combo.clear()
        self.network_client_combo.addItem("所有已连接客户端", None)
        for endpoint in endpoints:
            self.network_client_combo.addItem(endpoint, endpoint)
        index = self.network_client_combo.findData(selected)
        self.network_client_combo.setCurrentIndex(index if index >= 0 else 0)
        del blocker
        self._set_network_send_target()

    def _set_network_send_target(self, _index: int = 0):
        self.network_manager.set_send_target(self.network_client_combo.currentData())
    ...
    def toggle_connection(self):
        ...
        if self.current_mode == 'serial':
            ...
            try:
                ...
                self.on_error("请输入 1 到 65535 的端口")
                return
            host = self.network_host_input.text().strip()
            local_host = self.network_local_host_input.text().strip() or "0.0.0.0"
            mode = self._network_mode_key()
            if mode == TCP_SERVER:
                host = host or "0.0.0.0"
            elif not host:
                self.on_error("请输入目标主机")
                return
            self._on_network_config_changed()
            ...
            import threading
            threading.Thread(
                target=self.network_manager.connect,
                args=(host, port, mode, local_host, local_port),
                daemon=True,
            ).start()
    ...
    def on_data_received(self, data: bytes):
        ...
        self.update_stats()
        self._reply_to_network_message_if_needed()
    ...
    def _reply_to_network_message_if_needed(self):
        """Send one explicitly configured reply for server or UDP traffic."""
        if (
            self.current_mode != "network"
            or self._network_mode_key() == TCP_CLIENT
            or not self.network_auto_reply_check.isChecked()
        ):
            return
        text = self.network_auto_reply_input.text()
        format_type = self.network_auto_reply_format.currentText()
        analysis = analyze_payload(text, format_type)
        if not analysis.valid or not analysis.payload:
            return

        display = analysis.normalized if format_type == "HEX" else text
        self.display_sent_data(display, format_type, analysis.payload)
        import threading
        threading.Thread(
            target=lambda: self.manual_send_completed.emit(
                self.network_manager.send_data(analysis.payload), analysis.byte_count
            ),
            daemon=True,
        ).start()
    ...
    def on_device_connected(self):
        ...
        if self.current_mode == 'serial':
            ...
            self.status_label.setText("网络已连接")
            self.network_type_combo.setEnabled(False)
            self.network_host_input.setEnabled(False)
            self.network_port_input.setEnabled(False)
            self.network_local_host_input.setEnabled(False)
            self.network_local_port_input.setEnabled(False)
    ...
    def on_device_disconnected(self):
        ...
        if self.current_mode == 'serial':
            ...
            self.network_type_combo.setEnabled(True)
            self.network_host_input.setEnabled(True)
            self.network_port_input.setEnabled(True)
            self.network_local_host_input.setEnabled(True)
            self.network_local_port_input.setEnabled(True)
