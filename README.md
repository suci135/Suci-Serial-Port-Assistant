...
一个基于 PyQt6 的现代化串口、蓝牙与网络调试工具，采用 macOS 风格设计，支持 Windows。
...
- 蓝牙 BLE 通信，支持设备扫描与连接
- 网络调试：TCP 客户端、TCP 服务端、UDP、保活、广播与本机 IP/端口绑定
...
完整功能规划与分期进度见 [功能全集与实施路线图](docs/FEATURE_ROADMAP.md)。
...
2. 运行 `scripts\\build_installer.bat`
3. 安装包生成在 `installer_output\` 目录

打包脚本会自动生成浅色/深色安装向导视觉资源，并在常见安装路径中查找 Inno Setup。发布新版本时只需修改 `scripts/build_installer.bat` 顶部的 `APP_VERSION`。
...
└── src/
    ...
    │   └── network_manager.py   # TCP/UDP 网络管理
    ├── ui/
